...
knitr::kable(d3, caption = "Phylogenetic distance matrix showing pair-wise differences in hemoglobin hemoglobin ⍺1 sequences between all samples in the data set.") %>% kable_styling(font_size = 9) %>% kable_styling("striped") %>% scroll_box(width = "100%")
...
```{r out.width='100%', fig.cap="Neighbor-joining tree of different vertebrates based on hemoglobin ⍺1 sequences."}
...
If you are interested in learning more about reconstructing phylogenies, I recommend "*Phylogenetic Inference*" by Mark Holder, Chapter 12 in the [*Princeton Guide to Evolution*](https://k-state.primo.exlibrisgroup.com/permalink/01KSU_INST/1177os2/alma9942417312002401) that you can access for free through the K-State Library. The chapter briefly introduces parsimony, likelihood-based approaches, and Bayesian inference in more detail.
:::
